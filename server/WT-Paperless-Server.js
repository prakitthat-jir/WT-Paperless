...
function loadConfig(){
  ...
  if(CFG.requireCodeOnLan === undefined) CFG.requireCodeOnLan = false;
  /* ที่อยู่หน้าเว็บถาวรบน GitHub Pages + โฟลเดอร์ repo ที่ใช้ประกาศที่อยู่อุโมงค์ */
  if(CFG.pagesUrl === undefined){ CFG.pagesUrl = "https://prakitthat-jir.github.io/WT-Paperless/"; changed = true; }
  if(CFG.repoDir  === undefined){ CFG.repoDir  = "C:/Users/dissheet06/wt-paperless"; changed = true; }  /* ในออฟฟิศไม่ต้องใส่รหัส */
...
const SHEET = {
  ...
  forklift:   "ตรวจสอบรถโฟล์คลิฟท์"
};
const FORM_ORDER = ["vehicle","hygiene","glass","truckcheck","evaluate","forklift"];
...
function buildWorkbook(){
  ...
  /* เรียงตามประเภท (กล่อง → แผ่น) แล้วตามลำดับในผังรายชื่อ · คนที่ออกแล้วไว้ท้ายสุด */
  const conRank = t=> t==="BOX" ? 0 : t==="SHEET" ? 1 : 2;
  const conList = (M().contractors).slice().sort((a,b)=>
    (a.off?1:0)-(b.off?1:0) || conRank(a.type)-conRank(b.type)
    || (Number(a.ord)||999)-(Number(b.ord)||999) || (Number(a.code)||0)-(Number(b.code)||0));
  /* รถคันเดียวกันมีสองคน — เขียนกำกับไว้ในคอลัมน์สถานะ */
  const duoG = {}, duo = {};
  conList.filter(c=> !c.off).forEach(c=>{
    const p = String(c.plate||"").replace(/[s-]/g,"").toLowerCase();
    const k = p ? (c.type||"") + "|" + p : "เดี่ยว|" + c.code;
    (duoG[k] = duoG[k] || []).push(c);
  });
  Object.keys(duoG).forEach(k=>{
    const v = duoG[k].sort((a,b)=> (a.helper?1:0)-(b.helper?1:0) || (Number(a.ord)||999)-(Number(b.ord)||999));
    v.forEach((c,i)=> duo[c.code] = {n:v.length, i:i+1});
  });
  const conNote = c=>{
    if(c.off) return "ออกแล้ว";
    const d = duo[c.code];
    return (d && d.n > 1) ? ("รถคันนี้ " + d.n + " คน · คนที่ " + d.i) : "";
  };
  sheets.push({name:"ผู้รับเหมา", header: HEADERS["ผู้รับเหมา"],
    rows: conList.map((c,i)=>["", i+1, c.code, c.name, c.addr, c.contact, c.plate, c.tel1, c.tel2, "", c.type, "", "", conNote(c)])});
...
/* ฟอร์มรายเดือน (FM-TB-011): 1 ใบ = 1 เดือน -> เขียนลง Excel เป็นวันละ 1 แถว */
const FORK_ITEMS = ["1.1","1.2","2.1","2.2","2.3","2.4","3.1","3.2","4.1","4.2","4.3"];
function forkliftRows(){
  const list = (DB.records.forklift || []).slice().sort((a,b)=>
    String(a.month || a.date || "").localeCompare(String(b.month || b.date || "")) || (a.createdAt||0)-(b.createdAt||0));
  const out = [];
  list.forEach(r=>{
    const ym = String(r.month || String(r.date || "").slice(0,7) || "");
    const p = ym.split("-");
    if(p.length < 2 || !+p[0] || !+p[1]) return;
    const n = new Date(+p[0], +p[1], 0).getDate();
    for(let d=1; d<=n; d++){
      const vals = FORK_ITEMS.map(k=>{
        const o = (r.items || {})[k];
        return (o && typeof o === "object") ? (o[d] || "") : "";
      });
      if(!vals.some(v=> v !== "")) continue;
      const ser = toSerial(ym + "-" + pad(d));
      const vd  = toSerial(r.verifyDate);
      out.push(["", r.id || "", +p[0], +p[1], ser===""? "" : {t:"d", v:ser}, ""]
        .concat(vals)
        .concat([r.inspector || r.inspectorName || "",
                 r.verifier  || r.verifierName  || "",
                 vd===""? "" : {t:"d", v:vd}, "", ""]));
    }
  });
  return out;
...
function rowsFor(fid){
  if(fid === "forklift") return forkliftRows();
...
const handler = async (req,res)=>{
  ...
  try{
    ...
    if(url === "/api/ping") return send(res, 200, {ok:true, mode:"server", file:path.basename(DATA_XLSX),
                                                   ...
                                                   needsAuth, publicUrl:PUBLIC_URL || "", pagesUrl:(CFG && CFG.pagesUrl) || "",
                                                   hasBase:!!BASE});
    ...
    /* ---- ไฟล์ปกติ: อนุญาตเฉพาะรายการที่กำหนด (ที่เหลือปฏิเสธหมด) ----
       เดิมใช้วิธีบล็อกทีละไฟล์ ซึ่งเสี่ยงมาก เพราะไฟล์ข้อมูลที่ลืมใส่ในรายการ
       (เช่น .xlsx เดิม, WT-Paperless-Data.json, โฟลเดอร์สำรอง, รูปลายเซ็นเก่า)
       จะถูกดาวน์โหลดผ่านอุโมงค์ได้โดยไม่ต้องใส่รหัส */
    let rel = url === "/" ? APP_FILE : url.replace(/^\//,"");

    const SHELL_FILES = new Set([APP_FILE, "index.html", "WT-Paperless.webmanifest", "WT-Paperless-SW.js"]);
    const isShell = SHELL_FILES.has(rel);
    const isIcon  = /^WT-Paperless-Icons\/[A-Za-z0-9._-]+\.png$/.test(rel);
    const isSig   = /^WT-Paperless-Images\/[A-Za-z0-9._-]+\.(png|jpg|jpeg)$/i.test(rel);

    if(!isShell && !isIcon && !isSig){ res.writeHead(403); return res.end("forbidden"); }

    /* รูปลายเซ็นเป็นข้อมูลส่วนบุคคล — จากนอกออฟฟิศต้องแนบโทเคนมาด้วย
       (แท็ก <img> ส่ง header ไม่ได้ จึงรับผ่าน ?t=... แทน) */
    if(isSig && needsAuth){
      const q = new URLSearchParams((req.url.split("?")[1] || ""));
      if(!validToken(q.get("t") || "")){ res.writeHead(401); return res.end("unauthorized"); }
    }
...
/* ที่อยู่ถาวรที่ให้ทุกคนใช้ — หน้าเว็บบน GitHub Pages ซึ่งไม่เคยเปลี่ยน */
...
/* ============================================================
   ประกาศที่อยู่อุโมงค์ปัจจุบันขึ้น GitHub Pages (ไฟล์ api.json)
   ทำไม: อุโมงค์แบบไม่มีโดเมนได้ชื่อใหม่ทุกครั้งที่เปิดโปรแกรม
         พอเครื่องแม่บอกที่อยู่ล่าสุดไว้ที่เดิมเสมอ มือถือก็เปิด
         ลิงก์เดิมได้ตลอด ไม่ต้องสแกน QR ใหม่
   ============================================================ */
function repoDir(){
  const d = process.env.WT_REPO || (CFG && CFG.repoDir) || "";
  return d && fs.existsSync(path.join(d, ".git")) ? d : "";
}
function git(repo, args){
  return new Promise(done=>{
    execFile("git", ["-C", repo].concat(args),
      ...
      (err, so, se)=> done({ok:!err, out:(String(so||"") + String(se||"")).trim()}));
  });
}
function firstLine(s){ return String(s||"").split("\n")[0]; }
async function publishApiUrl(u){
  const repo = repoDir();
  if(!repo){
    console.log("   ! ประกาศที่อยู่ไม่ได้ — ไม่พบโฟลเดอร์ repo (ตั้ง repoDir ใน WT-Paperless-Config.json)");
    return false;
  }
  const file = path.join(repo, "api.json");
  try{
    if(fs.existsSync(file)){
      const old = JSON.parse(fs.readFileSync(file, "utf8"));
      if(old && old.api === u) return true;            /* ที่อยู่เดิม ไม่ต้องส่งซ้ำ */
    }
    fs.writeFileSync(file, JSON.stringify({api:u, updated:new Date().toISOString()}, null, 2), "utf8");
  }catch(e){
    console.log("   ! เขียน api.json ไม่ได้:", e.message);
    return false;
  }
  const add = await git(repo, ["add", "api.json"]);
  if(!add.ok){ console.log("   ! git add ไม่ผ่าน:", firstLine(add.out)); return false; }
  const com = await git(repo, ["commit", "-m", "ที่อยู่อุโมงค์ปัจจุบัน (อัตโนมัติ)", "--", "api.json"]);
  if(!com.ok && !/nothing to commit|no changes added/i.test(com.out)){
    console.log("   ! git commit ไม่ผ่าน:", firstLine(com.out));
    return false;
  }
  const push = await git(repo, ["push", "origin", "HEAD"]);
  if(!push.ok){
    console.log("   ! ส่งขึ้น GitHub ไม่สำเร็จ — มือถือจะยังเห็นที่อยู่เก่า");
    console.log("     " + firstLine(push.out));
    return false;
  }
  return true;
}
...
server.listen(PORT, "0.0.0.0", ()=>{
  ...
  TUNNEL = startTunnel(async u=>{
    const perm = permanentLink();
    console.log("");
    console.log("  ============================================================");
    console.log("   เปิดจากที่ไหนก็ได้ (ไม่ต้องอยู่ Wi-Fi เดียวกัน)");
    console.log("  ============================================================");
    if(perm){
      console.log("   ที่อยู่ถาวร :  " + perm);
      console.log("   รหัส        :  " + CFG.accessCode);
      console.log("");
      console.log("   * ลิงก์นี้ไม่เปลี่ยนอีกแล้ว — มือถือติดตั้งครั้งเดียวใช้ได้ตลอด");
      console.log("   * กำลังบอกที่อยู่ล่าสุดให้ลิงก์ถาวรรู้…");
      const ok = await publishApiUrl(u);
      if(ok) console.log("   * เรียบร้อย — GitHub ใช้เวลาอัปเดตราว 1 นาที แล้วมือถือจะต่อได้เอง");
      else   console.log("   * ระหว่างที่ยังประกาศไม่ได้ ให้ใช้ลิงก์ชั่วคราวนี้ :  " + shareLink());
    } else {
      console.log("   ที่อยู่ :  " + u);
      console.log("   รหัส   :  " + CFG.accessCode);
      console.log("");
      console.log("   * ยังไม่ได้ตั้ง pagesUrl ใน WT-Paperless-Config.json");
      console.log("     ที่อยู่นี้จึงเปลี่ยนใหม่ทุกครั้งที่เปิดโปรแกรม");
    }
  });
